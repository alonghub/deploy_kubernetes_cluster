...
注意：
1. 如果没有特殊指明，本文档的所有操作**均在 along-k8s-01 节点上执行**；
2. flanneld 与 etcd v3.4.x 不兼容，如果要安装 flanneld（本文档使用 calio），则需要将 etcd **降级到 v3.3.x 版本**；本文直接安装的 etcd v3.3.x
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp etcd-v3.3.25-linux-amd64/etcd* root@${node_ip}:/opt/k8s/bin
...
```
+ 3.3.25 版本的 etcd/etcdctl 默认启用了 V3 API，所以执行 etcdctl 命令时不需要再指定环境变量 `ETCDCTL_API=3`； 
...
``` bash
>>> 192.168.200.102
https://192.168.200.102:2379 is healthy: successfully committed proposal: took = 2.756451ms
>>> 192.168.200.103
https://192.168.200.103:2379 is healthy: successfully committed proposal: took = 2.025018ms
>>> 192.168.200.104
https://192.168.200.104:2379 is healthy: successfully committed proposal: took = 2.335097ms
```
...
+-----------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
| https://192.168.200.102:2379 | 4250b255e93e0076 |   3.3.25 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
| https://192.168.200.103:2379 | b3d912e6166f1213 |   3.3.25 |   20 kB |      true |      false |         2 |          8 |                  8 |        |
| https://192.168.200.104:2379 | 8a4d4a2904de8446 |   3.3.25 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
+-----------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
```
