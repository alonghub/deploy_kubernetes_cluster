...
health:
  storagedriver:
    ...
    threshold: 3
EOF
[k8s@along-k8s-01 cert]$ cp config.yml registry
[k8s@along-k8s-01 cert]$ scp -r registry 172.27.132.67:/opt/k8s
```
...
``` bash
[k8s@along-k8s-01 cert]$ sudo mkdir -p /etc/docker/certs.d/172.27.132.67:8000
[k8s@along-k8s-01 cert]$ sudo cp /etc/kubernetes/cert/ca.pem /etc/docker/certs.d/172.27.132.67:8000/ca.crt
```
...
[k8s@along-k8s-01 cert]$ python s3test.py
...
1. ceph-deploy config push along-k8s-01 along-k8s-02 along-k8s-03
1.  systemctl restart 'ceph-mds@along-k8s-03.service'
    systemctl restart ceph-osd@0
    systemctl restart 'ceph-mon@along-k8s-01.service'
    systemctl restart 'ceph-mgr@along-k8s-01.service'
...
[root@along-k8s-01 ~]# docker login  172.27.132.67:8000
Username: foo
Password:
